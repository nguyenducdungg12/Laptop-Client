...
function Register(props) {
    ...
    return (
        <form style={display == 3 ? { display: "block" } : { display: "none" }} onSubmit={onSubmitForgotPassword}>
              <div class="modal-main__title--container">
                <div class="modal-main__title--container__child "style={{width:"100%"}} >
                    <h1 class="modal-main__title">Quên mật khẩu</h1>
                </div>
            </div>
            <div class="modal__body__sign-in__form modal__body__sign-in__form__DK" style={display == 3 ? { display: "flex" } : { display: "none" }}>
                <input type="email" class="modal__body__sign-in__form-input" name="email" value={Email} required placeholder="Nhập Email" onChange={onChangeForm} />
